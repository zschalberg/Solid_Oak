...
def load_grass_metatile_ids(attr_path, id_offset):
...
def main():
    ...
    grass_ids = load_grass_metatile_ids(tileset_attr_paths[primary_symbol], 0)
    grass_ids |= load_grass_metatile_ids(
    ...
    # metatile graphic tagged as tall grass to choose between.
    if len(grass_ids) > 1:
        grass_choices = sorted(grass_ids)
        for i in range(0, len(data), 2):
            cell = data[i] | (data[i + 1] << 8)
            metatile_id = cell & MAPGRID_METATILE_ID_MASK
            if metatile_id in grass_ids:
                new_id = random.choice(grass_choices)
